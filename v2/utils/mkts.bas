...
Dim Shared As Integer verbose, silent, mainIdx, cPoolIndex, tMapsIndex, flipped, upsideDown, clrIdx, blackiszero
...
Sub usage
	...
	Print "               text is special chars with text output in .h. Ask na_th_an"
	Print "               tiles does straight 16x16 conversion -> binary, v-h ordered"
	Print "               coltiles is same as tiles, but h-v ordered"
	...
	Print "blackiszero    Every blank (all 0s) tile will be encoded as tile 0"
	Print
...
Sub nesDoTexts (img As Any Ptr, pal () As Integer, xOrg As Integer, yOrg As Integer, wProc As Integer, hProc As Integer, tsMapFn As String, label As String, offset As Integer, max As Integer)
	Dim As Integer x0, y0
	Dim As Integer x1, y1
	Dim As Integer x, y
	Dim As Integer ct
	...
	While y <= y1
		...
		If Point (x, y, img) = &HFF00FF00 Then
			Print #fOut, "// Skipped line { ";
			x = x + 8
		Else
			Print #fOut, "const unsigned char str_" & label & "_" & Lcase (Hex (strCtr, 2)) & " [] = { ";
			strCtr = strCtr + 1
		End If
...
Sub doColTiles (img As Any Ptr, pal () As Integer, xOrg As Integer, yOrg As Integer, wProc As Integer, hProc As Integer, wMeta As Integer, hMeta As Integer, tsMapFn As String, label As String, max As Integer)
	Dim As Integer x0, y0, x1, y1, x, y, ct, ctt, fOut, xx, yy, xa, ya, wp
	Dim As Integer hSize, vSize
	Dim As uByte pattern (15)
...
Function findPatternInPoolAndAddEx (patternS As String, ByRef wasNew As Integer, ByRef modifier As Integer) As Integer
	Dim As Integer res, i
	Dim As String patternSH, patternSV, patternSHV
	
	If blackiszero And isAllZeroes (patternS) Then
		findPatternInPoolAndAddEx = 9999
		Exit Function
	End If
	...
	' Return
	wasNew = -1
	findPatternInPoolAndAddEx = cPoolIndex - 1
End Function
		...
		For x = x0 To x1 Step hSize
			...
			For yy = 1 To hMeta
				...
				For xx = 1 To wMeta
					...
					nesExtractPatternFrom img, xa, ya, pal (), pattern ()
					patternS = patternToString (pattern ())
					ct = findPatternInPoolAndAddEx (patternS, wasnew, modifier)
					If wasnew Then copyArrayToMainBin pattern ()
					If ct = 9999 Then tMap (tMapI) = 0 Else tMap (tMapI) = ct + offset
					metapals (tMapI) = wp Or modifier
					tMapI = tMapI + 1
					xa = xa + 8
				Next xx
				ya = ya + 8
			...
			' Write upside-down metaspriteset ONLY EVEN!
			If upsideDown And ((tMapsIndex And 1) = 0) Then
...
filteredPuts "mkts v0.2Yun"
filteredPuts "OJAL! Este hace fullerías Yun. No lo uses por ahí."
...
If sclpGetValue ("mode") = "coltiles" Or sclpGetValue ("mode") = "tiles" Or sclpGetValue ("mode") = "mapped" Or sclpGetValue ("mode") = "sprites" Or sclpGetValue ("mode") = "text" Or sclpGetValue ("mode") = "sprites16" Then
	If sclpGetValue ("tsmap") = "" Then
		filteredPuts ("**ERROR** mode=tiles, coltiles, mapped, sprites, sprites16, text need tsmap")
		filteredPuts ("")
...
If platform = PLATFORM_NES Then
	Select Case sclpGetValue ("mode")
		...
		Case "coltiles":
			doColTiles img, pal (), xOrg, yOrg, wProc, hProc, wMeta, hMeta, sclpGetValue ("tsmap"), sclpGetValue ("label"), max			
		Case "mapped":

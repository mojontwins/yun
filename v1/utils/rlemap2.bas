...
Dim Shared As Integer coords (10)

Sub usage
	Print "$ rlemap.exe in=in.map out=out.h size=w,h prefix=prefix [tlock=t] [main=a,b]"
	Print "             [scrsizes]"
	Print
	Print "in              Is the input file."
	Print "out             Is the output file, c code."
	Print "size            (w, h) size of your map in 16x12 screens."
	Print "tlock           tile lock #, can be omitted (so no locks are detected)"
	Print "main            [a, b] interval of tiles to be considered main. 16 tiles max."
	Print "scrsizes        Byte size for each screen added as remarks."
End Sub
...
' Interval
If sclpGetValue ("main") <> "" Then
	coordinatesStringToArray sclpGetValue ("main"), coords ()
	t0 = coords (0)
	t1 = coords (1)
Else
	t0 = 0: t1 = 15
End If
If t1 - t0 > 15 Then 
	Print "Too many tiles in interval (" & (t1 - t0 + 1) & ")"
	usage
	End
End If
...
' Write output
f = Freefile
Open sclpGetValue ("out") For Output As #f
...
Print #f, "#define OFFSET_MAIN_TS_BY_" & Ucase (prefix) & " " & t0
Print #f, ""
...
For nPant = 0 To mapPants - 1
	...
	Else
		...
		Print #f, " };"
	End If
	If sclpGetValue ("scrsizes") <> "" Then Print #f, "// Screen size = " & scrsize & " bytes."
Next nPant
Print #f, ""
